...
async def get_thumb(videoid):
    ...
    try:
        ...
        # Crop and prepare logo with rounded corners (no colored border)
        ...
        mask_draw.rounded_rectangle([(0, 0), (520, 520)], radius=0, fill=255)
        logo = ImageOps.fit(logo, (520, 520), centering=(0.5, 0.5))
        logo.putalpha(mask)
        logo_pos_x = (image2.width - 520) // 2  # Center logo horizontally
        logo_pos_y = (image2.height - 520) // 2  # Center logo vertically
        background.paste(logo, (logo_pos_x, logo_pos_y), logo)

        draw = ImageDraw.Draw(background)
        font = ImageFont.truetype("AnonXMusic/assets/font3.ttf", 35)  # Title
        font2 = ImageFont.truetype("AnonXMusic/assets/font3.ttf", 50)  # Now Playing
        ...
        gap = 15
        box_gap = 20
        max_box_width = 350  # Ensure boxes stay clear of logo (x=380)

        # Prepare text with dynamic truncation
        title = truncate_text(title, 60, add_ellipsis=True)
        ...
        # Wrap title text
        para = textwrap.wrap(title, width=25)  # Tighter wrap for title
        ...
        for i, line in enumerate(text_lines):
            ...
            text_width = min(bbox[2] - bbox[0], max_box_width - 2 * padding)  # Constrain width
            text_height = bbox[3] - bbox[1]
            text_heights.append(text_height)
            text_widths.append(text_width)

        # Now Playing box
        now_playing_width = min(text_widths[0] + 2 * padding + 20, max_box_width)
        now_playing_height = text_heights[0] + 2 * padding
        ...
        now_playing_box = now_playing_box.filter(ImageFilter.GaussianBlur(3))
        ...
        title_height = sum([text_heights[i] for i in [1, 2] if i < len(text_heights)]) + (len(title_lines) - 1) * gap + 2 * padding
        ...
        title_box = title_box.filter(ImageFilter.GaussianBlur(3))
        ...
        info_height = sum([text_heights[i] for i in range(3, len(text_heights))]) + (len(info_lines) - 1) * gap + 2 * padding
        ...
        info_box = info_box.filter(ImageFilter.GaussianBlur(3))
        ...
        for i, line in enumerate(title_lines):
            ...
            current_y += text_heights[i + 1] + gap
        ...
        for i, line in enumerate(info_lines):
            ...
            current_y += text_heights[i + 3] + gap
